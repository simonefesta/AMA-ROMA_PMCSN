...
/**
 * rappresenta la msq per l'gommista
 */
public class ControllerGommista {
    ...
    public ControllerGommista(){
        ...
        this.eventListGommista.set(0,new EventListEntry(event.getT(), event.getX(), event.getVehicleType()));
    ...
    public void baseSimulation() throws Exception {
        int e;
        //prende la lista di eventi per l'gommista
        ...
        while(eventHandler.getInternalEventsGommista().size()>0 || this.number>0){
            ...
            if(e==0){ // controllo se l'evento è un arrivo
                ...
                if(this.number<=SERVERS_GOMMISTA){ //controllo se ci sono server liberi
                    ...
                    this.s=findOneServerIdle(eventListGommista); //ottengo l'indice di un server libero
                    ...
                    eventListGommista.get(s).setT(this.time.getCurrent()+service);
                    eventListGommista.get(s).setX(1);
                    eventListGommista.get(s).setVehicleType(vType);
                ...
                EventListEntry event=eventListGommista.get(s);
                ...
                if(this.number>=SERVERS_GOMMISTA){ //controllo se ci sono altri eventi da gestire
                    ...
                    double service=this.rnd.getService(queueGommista.get(0).getVehicleType());
                    ...
                    eventListGommista.get(s).setT(this.time.getCurrent()+service);
                    eventListGommista.get(s).setVehicleType(queueGommista.get(0).getVehicleType());
                    ...
                    eventListGommista.get(e).setX(0);
                    //aggiorna la lista
                    this.eventHandler.setEventsGommista(eventListGommista);
                }
